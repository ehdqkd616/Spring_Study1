...
<head>
<meta charset="UTF-8">
<title>스프링 게시판 내용보기</title>
<%-- jQuery 라이브러리 읽어옴. --%>
<script src="../resources/js/jquery.js"></script>
<%-- ./는 현재경로, ../는 한단계 상위경로를 뜻한다. --%>
<style>
 #modDiv{
...
<body>
  <table border="1">
   ...
   <tr>
    ...
    <%-- ${b.title}은 EL 즉 표현언어로써 자바 코드로 표현하자면 b.getTitle()과 같다. --%>
   </tr>
   ...
   <tr>
    ...
    <%-- board_edit?bno=번호값 get방식으로 전달. --%>
    
    <input type="button" value="삭제"
    onclick="location=
    	'/controller/board/board_del?bno=${b.bno}';" />
...
<script>
 var bno=${b.bno};//게시판 번호값을 bno변수에 저장시킴
 //자바스크립트에서 EL 즉 표현언어를 사용할 수 있다.
 ...
 function getAllList(){
	 $.getJSON("/controller/replies/all/"+bno,function(data){
		...
		$('#replies').html(str);//#replies 아이디영역에 html()함수 사용하여 댓글
		//목록을 변경적용. html()함수는 문자와 태그를 함께 변경적용.
	 });
 }//getAllList() 함수
 ...
 $('#replyAddBtn').on('click',function(){
   ...
   $.ajax({//get or post방식을 처리하는 jQuery 비동기식 아작스 함수=>사용빈도가 가장높다
	   ...
	   success:function(result){
		   ...
		   if(result=='success'){
			   alert('댓글이 등록되었습니다!');
			   location.reload();//자바 스크립트에서 reload()메서드는 새로고침(F5)
